...
def make_rebalance_dates(index, start_date, interval=42):
...
def build_positions_market_only(target_weights, rebalance_dates, market_exit_ok, max_gross_exposure):
    ...
    market_mask = market_exit_ok.shift(1).fillna(False).astype(bool)

    for date in target_weights.index:

        if not market_mask.loc[date]:
            current_positions[:] = 0.0

        positions.loc[date] = current_positions

        if rebalance_dates.loc[date]:
            if market_mask.loc[date]:
...
def run_relative_strength_backtest(symbols=None, start_date='2018-01-01', end_date='2025-01-01',
    ...
    positions = build_positions_market_only(
        target_weights=target_weights,
        rebalance_dates=rebalance_dates,
        market_exit_ok=market_exit_ok,
    ...
    print_stats("Relative Strength Momentum - Market Exit Only", strategy_stats)
    ...
    print("Exit Type: Market only, no individual stock daily exits")
